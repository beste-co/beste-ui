  ...
  slides: [
    {
      ...
      buttons: [{ id: "btn-1", label: "Start Creating", href: "https://beste.co" }],
    },
    {
      id: "slide-2",
      ...
      buttons: [{ id: "btn-1", label: "Start Free Trial", href: "https://beste.co" }],
    },
    {
      id: "slide-3",
      ...
      buttons: [{ id: "btn-1", label: "Join Waitlist", href: "https://beste.co" }],
    },
  ],
  autoPlay: true,
  interval: 5000,
...
export function Hero20({
  ...
  return (
    <section
      className={cn(
        "relative w-full lg:min-h-[600px] lg:h-screen overflow-hidden bg-background",
        className
      )}
    ...
    >
      {/* Main Content Grid */}
      <div className="relative lg:h-full mx-auto max-w-7xl px-4 md:px-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 lg:h-full lg:items-center py-12 lg:py-0">
              ...
              >
                ...
                {currentSlide.badge && (
                  <motion.div variants={contentVariants} custom={0}>
                    <Badge variant={currentSlide.badge.variant ?? "default"} className="w-fit">
                ...
                {currentSlide.heading && (
                  <motion.h1
                    ...
                    custom={1}
                    className="text-4xl sm:text-5xl lg:text-6xl xl:text-7xl font-bold tracking-tight leading-[1.1] text-foreground"
                  >
                    {currentSlide.heading}
                  </motion.h1>
                )}
                ...
                {currentSlide.description && (
                  <motion.p
                    variants={contentVariants}
                    custom={2}
                    className="text-base sm:text-lg lg:text-xl max-w-lg leading-relaxed text-muted-foreground"
                  >
                    {currentSlide.description}
                  </motion.p>
                )}
          ...
          <div className="relative order-1 lg:order-2 h-[300px] sm:h-[400px] lg:h-[70%]">
            ...
            <div className="relative w-full h-full rounded-2xl lg:rounded-3xl overflow-hidden shadow-2xl">
              {/* Border Effect */}
              <div className="absolute inset-0 rounded-2xl lg:rounded-3xl border z-10 pointer-events-none" />
              ...
              {showArrows && slideCount > 1 && (
                <div className="absolute inset-x-4 top-1/2 -translate-y-1/2 flex justify-between z-10 pointer-events-none">
                  <button
                    type="button"
                    onClick={() => paginate(-1)}
                    className="size-10 lg:size-12 rounded-full bg-background/80 backdrop-blur-md border flex items-center justify-center text-foreground hover:bg-background transition-all duration-300 pointer-events-auto group/hero20-prev"
                  >
                    <ChevronLeft className="size-5 group-hover/hero20-prev:-translate-x-0.5 transition-transform" />
                  </button>
                  <button
                    type="button"
                    onClick={() => paginate(1)}
                    className="size-10 lg:size-12 rounded-full bg-background/80 backdrop-blur-md border flex items-center justify-center text-foreground hover:bg-background transition-all duration-300 pointer-events-auto group/hero20-next"
                  >
                    <ChevronRight className="size-5 group-hover/hero20-next:translate-x-0.5 transition-transform" />
                  </button>
                </div>
              )}
            </div>
            ...
            {showThumbnails && slideCount > 1 && (
              <div className="absolute -bottom-4 left-1/2 -translate-x-1/2 lg:-bottom-6 flex items-center gap-2 lg:gap-3 p-2 rounded-xl bg-card backdrop-blur-xl border">
            ...
            >
              <div
                className={cn(
                  ...
                  idx === currentIndex ? "h-12 bg-muted" : "h-6 bg-muted/50 hover:bg-muted"
                )}
              >
                {idx === currentIndex && (
                  <motion.div
