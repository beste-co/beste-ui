  ...
  hotspots: [
    {
      ...
      title: "Quick Actions",
      description: "Access frequently used features with one click from the sidebar.",
      size: "lg",
      appearance: "light",
    },
    {
      ...
      description: "Your data is encrypted end-to-end and stored in secure servers.",
      size: "lg",
      appearance: "light",
    },
    {
      ...
      title: "Smart Reports",
      description: "AI-generated reports that highlight the most important metrics.",
      size: "lg",
      appearance: "light",
    },
  ],
    ...
    <Popover>
      <PopoverTrigger asChild>
        ...
        >
          ...
          >
            {hotspot.icon || <span className="text-xs font-bold">{hotspot.id}</span>}
          </span>
        </button>
      </PopoverTrigger>
      <PopoverContent
        ...
        align="center"
        sideOffset={12}
        className={cn("w-72 p-4 rounded-lg", "bg-background/95 backdrop-blur-md border shadow-2xl")}
      >
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            {hotspot.icon && (
              <span className="p-2 rounded-lg bg-primary/10 text-primary">{hotspot.icon}</span>
            )}
            <h4 className="font-semibold text-base">{hotspot.title}</h4>
          </div>
          <p className="text-sm leading-relaxed text-muted-foreground">{hotspot.description}</p>
          {hotspot.link && (
            <Link
      ...
      <div className="mx-auto max-w-6xl px-4 md:px-6">
        ...
        >
          ...
          <div className="flex flex-col gap-6 justify-center">
            ...
            {heading && (
              <h2 className="text-3xl md:text-4xl lg:text-5xl font-semibold">{heading}</h2>
            )}
            ...
            {buttons.length > 0 && (
              <div className="flex flex-wrap gap-3 mt-2">
                {buttons.map((button) => (
                  <Button key={button.id} variant={button.variant ?? "default"} size="lg" asChild>
