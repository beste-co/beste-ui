...
export const hero19Demo: Hero19Props = {
  slides: [
    ...
    },
    {
      ...
      buttons: [{ id: "btn-1", label: "View Portfolio", href: "https://beste.co" }],
    },
    {
      id: "slide-3",
      ...
      buttons: [{ id: "btn-1", label: "Start Building", href: "https://beste.co" }],
    },
  ],
  autoPlay: true,
  interval: 6000,
...
export function Hero19({
  ...
  return (
    <section
      className={cn("relative w-full min-h-[600px] h-screen overflow-hidden bg-black", className)}
      ...
      {showPreview && nextSlide && slideCount > 1 && (
        ...
        >
          ...
          <div className="relative w-28 h-16 lg:w-36 lg:h-20 rounded-lg overflow-hidden border border-white/20 group-hover:border-white/40 transition-all duration-300 group-hover:scale-105">
            {nextSlide.image && (
              <img
                className="absolute inset-0 size-full object-cover opacity-60 group-hover:opacity-100 transition-all duration-500 group-hover:scale-110"
          ...
          {slides.map((_, idx) => (
            <button
              ...
              className={cn(
                ...
                idx === currentIndex ? "w-8 h-2 bg-white" : "w-2 h-2 bg-white/30 hover:bg-white/50"
              )}
            />
          ))}
        </div>
