...
import { RegistryComponentShowcase } from "../../components/registry-component-showcase";
...
export default async function RegistryComponentDetailPage({ params }: PageProps) {
  ...
  return (
    ...
    <div className="mx-auto max-w-6xl px-4 py-8 md:px-6 md:py-12">
      ...
      {/*
        Content and rail. Below `lg` the rail is not rendered at all: a 200px sticky
        column has nowhere to stick on a phone, and a table of contents there is just
        a list the reader has to scroll past to reach the thing it lists.
      */}
      <div className="flex flex-col gap-10 lg:flex-row lg:items-start">
        ...
        {/*
          The same type scale the landing page uses: a title that carries the page
          rather than labels it, and a line under it at reading size. The
          description is no longer clamped — at this width two lines was a
          sentence cut off mid-thought, and these are one sentence each anyway.

          The buttons share a row with the title alone. Beside the paragraph too,
          they took the width they wanted and left the description whatever was
          over, which on a phone was half the screen and six ragged lines.
        */}
        <header className="mb-10 md:mb-12">
          <div className="flex items-start justify-between gap-4">
            ...
            {/*
              The arrows are here for someone browsing the set, and repeated at the
              bottom with names for someone who has read this one to the end. The two
              answer different moments, so both are worth having.
            */}
            <div className="flex shrink-0 items-center gap-1">
              <FavoriteButton
                registryComponentName={component.name}
                className="hidden sm:inline-flex"
              />
              <ThemePicker className="hidden sm:inline-flex" />
              {/* The neighbour's name when there is one — the arrows are the
                  only thing here a reader cannot see the destination of. */}
              <NavArrowButton
                direction="prev"
          ...
          {/*
            One showcase for every component now. Cards and dashboard panels used
            to get an iframe of their own, which gave two different documentation
            pages depending on the category a component happened to be filed
            under; the stage scales a large surface down instead.
          */}
          <RegistryComponentShowcase name={component.name} source={source} variant="page" />
          ...
          {/*
            The pager, at the end, where a reader who has finished is. It carries the
            neighbour's name rather than a bare arrow: two arrows in a corner ask you
            to guess where they go, and the guess is the reason they were never
            pressed.
          */}
          {(prev || next) && (
